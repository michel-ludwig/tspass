...
void term_ReplaceConstant(TERM Term, SYMBOL Symbol, SYMBOL Repl)
/**************************************************************
  INPUT:   A term, a constant symbol and a replacement symbol.
  RETURNS: void
  EFFECT:  All constants with <Symbol> in <Term> are replaced 
           by <Repl>
  CAUTION: 
***************************************************************/
{
...
#ifdef CHECK
  if (!term_IsTerm(Term) || !symbol_IsConstant(Symbol)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_ReplaceConstant: Illegal input.");
    misc_FinishErrorReport();
  }
#endif
  
  if (symbol_Equal(term_TopSymbol(Term), Symbol)) {
    term_RplacTop(Term,Repl);
  }
  else
    for (Scan = term_ArgumentList(Term); !list_Empty(Scan); Scan = list_Cdr(Scan))
      term_ReplaceConstant(list_Car(Scan),Symbol,Repl);
}
