...
typedef struct {
  SYMBOL    loopSearchConst; /* s_i^L */
  NAT       index; /* the 'i' */
  SYMBOL    eventualitySymbol; /* the 'L' */
} LOOPSEARCH_CONST;

typedef struct {
  SYMBOL   eventualitySymbol; /* L or l */
/*  SYMBOL   searchConstSymbol;  c^L or c^l  */
  SYMBOL*  map; /* a mapping from marker indexes to symbols */
  NAT      mapSize;
  NAT      markerCount; /* the number of markers created so far */
} EVENTUALITY_INF;
...
typedef struct PROOFSEARCH_HELP {
  ...
  int                   keptclauses;
  int                   derivedclauses;
  int                   successfulloopsearches;
  int                   forwardsubsumedclauses;
  int                   backwardsubsumedclauses;
  int                   differentmarkerclauses;
  int                   tautologyclauses;
  int                   loops;
  int                   backtracked;
  NAT                   nontrivclausenumber;
  int                   loopsearchiterations;
  EVENTUALITY_INF*    evloopsearchinf;
...
static __inline__ LIST prfs_EventualityClauses(PROOFSEARCH Prf)
{
  return Prf->eventualityclauses;
}

static __inline__ void prfs_SetEventualityClauses(PROOFSEARCH Prf, LIST Clauses)
{
  Prf->eventualityclauses = Clauses;
}
...
static __inline__ void prfs_SetPrecedence(PROOFSEARCH Prf, PRECEDENCE Prec)
{
  Prf->precedence = Prec;
}
...
static __inline__ void prfs_SetStore(PROOFSEARCH Prf, FLAGSTORE Store)
{
  Prf->store = Store;
}
...
static __inline__ void prfs_SetKeptClauses(PROOFSEARCH Prf, int k)
{
  Prf->keptclauses = k;
}
...
static __inline__ void prfs_SetDerivedClauses(PROOFSEARCH Prf, int k)
{
  Prf->derivedclauses = k;
}

static __inline__ int prfs_InputClauses(PROOFSEARCH Prf)
{
  return Prf->inputclauses;
}

static __inline__ void prfs_IncInputClauses(PROOFSEARCH Prf, int k)
{
  Prf->inputclauses += k;
}

static __inline__ void prfs_SetInputClauses(PROOFSEARCH Prf, int k)
{
  Prf->inputclauses = k;
}

static __inline__ int prfs_SuccessfulLoopSearches(PROOFSEARCH Prf)
{
  return Prf->successfulloopsearches;
}

static __inline__ void prfs_IncSuccessfulLoopSearches(PROOFSEARCH Prf)
{
  ++Prf->successfulloopsearches;
}

static __inline__ int prfs_ForwardSubsumedClauses(PROOFSEARCH Prf)
{
  return Prf->forwardsubsumedclauses;
}

static __inline__ void prfs_IncForwardSubsumedClauses(PROOFSEARCH Prf, int k)
{
  Prf->forwardsubsumedclauses += k;
}

static __inline__ void prfs_SetForwardSubsumedClauses(PROOFSEARCH Prf, int k)
{
  Prf->forwardsubsumedclauses = k;
}

static __inline__ int prfs_BackwardSubsumedClauses(PROOFSEARCH Prf)
{
  return Prf->backwardsubsumedclauses;
}

static __inline__ void prfs_IncBackwardSubsumedClauses(PROOFSEARCH Prf, int k)
{
  Prf->backwardsubsumedclauses += k;
}

static __inline__ void prfs_SetBackwardSubsumedClauses(PROOFSEARCH Prf, int k)
{
  Prf->backwardsubsumedclauses = k;
}

static __inline__ int prfs_DifferentMarkerClauses(PROOFSEARCH Prf)
{
  return Prf->differentmarkerclauses;
}

static __inline__ void prfs_IncDifferentMarkerClauses(PROOFSEARCH Prf, int k)
{
  Prf->differentmarkerclauses += k;
}

static __inline__ void prfs_SetDifferentMarkerClauses(PROOFSEARCH Prf, int k)
{
  Prf->differentmarkerclauses = k;
}

static __inline__ int prfs_TautologyClauses(PROOFSEARCH Prf)
{
  return Prf->tautologyclauses;
}

static __inline__ void prfs_IncTautologyClauses(PROOFSEARCH Prf, int k)
{
  Prf->tautologyclauses += k;
}

static __inline__ void prfs_SetTautologyClauses(PROOFSEARCH Prf, int k)
{
  Prf->tautologyclauses = k;
}
...
static __inline__ void prfs_SetEventualityLoopSearchInformation(PROOFSEARCH Prf,
                                                    EVENTUALITY_INF *inf)
{
  Prf->evloopsearchinf = inf;
}

static __inline__ int prfs_LoopSearchIterations(PROOFSEARCH Prf)
{
  return Prf->loopsearchiterations;
}

static __inline__ void prfs_ResetLoopSearchIterations(PROOFSEARCH Prf)
{
  Prf->loopsearchiterations = 0;
}

static __inline__ void prfs_IncreaseLoopSearchIterations(PROOFSEARCH Prf)
{
  ++Prf->loopsearchiterations;
}
