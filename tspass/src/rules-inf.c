...
***************************************************************/
{
  ...
  TERM Atom;
  int i,n;
  BOOL GivenContainsLoopSearchMarker = FALSE;
#ifdef CHECK
  if (!clause_IsClause(GivenClause, Flags, Precedence)) {
    misc_StartErrorReport();
  ...
  if(tl_ContainsLoopSearchMarker(GivenCopy)) {
    GivenContainsLoopSearchMarker = TRUE;
  }

  if(flag_UnorderedFineGrainedStepResolution(Flags)) {
     Ordered = FALSE;
  }
  ...
  for (i = clause_FirstAntecedentLitIndex(GivenCopy); i <= n; i++) {
    ...
    if(GivenContainsLoopSearchMarker) {
      if(clause_LiteralIsNegative(ActLit) && tl_IsUniversalLiteral(ActLit)) {
        continue;
      }
    }

    if(flag_UnorderedFineGrainedStepResolution(Flags)
       && clause_GetTemporalType(GivenClause) == STEP
       && tl_IsUniversalLiteral(ActLit)) {
        continue;
    }
	  ...
	  if (!term_IsVariable(PartnerAtom)) {	  
	    ...
	    for (LitList =  sharing_NAtomDataList(PartnerAtom); 
		 !list_Empty(LitList);  LitList = list_Cdr(LitList)) {
	      ...
	      if(flag_UnorderedFineGrainedStepResolution(Flags)
		&& clause_GetTemporalType(PartnerClause) == STEP && tl_IsUniversalLiteral(PartnerLit)) {
		  continue;
	      }
        ...
        if(clause_LiteralIsNegative(PartnerLit)) {
          if(tl_ContainsLoopSearchMarker(PartnerClause) && tl_IsUniversalLiteral(PartnerLit)) {
            continue;
          }
        }
...
***************************************************************/
{
  ...
  /* Check whether the given clause is a monadic negative universal clause 
     and modify the parameters accordingly. */
  if(tl_IsMostMonadicNegativeUniversalClause(GivenClause)) {
    Ordered = FALSE;
    Left = TRUE;
  }
