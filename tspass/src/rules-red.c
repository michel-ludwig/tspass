...
BOOL red_ExplicitTautology(PROOFSEARCH Search, CLAUSE Clause, FLAGSTORE Flags, 
...
***********************************************************/
{
  ...
  if(Result) {
    prfs_IncTautologyClauses(Search, 1);
  }
...
static BOOL red_Tautology(PROOFSEARCH Search, CLAUSE Clause, FLAGSTORE Flags, 
			  PRECEDENCE Precedence)
/**********************************************************
  INPUT:   A non-empty clause, a flag store and a
           precedence.
  ...
  while (CandTerm) { /* First check units */
    if (!term_IsVariable(CandTerm)) { /* Has to be an Atom! */
      ...
      while (!list_Empty(LitScan)) {
        ...
        CLAUSE PartnerClause = clause_LiteralOwningClause(NextLit);

        if(!TEMPORAL_MODE || !((clause_GetTemporalType(PartnerClause) == LOOPSEARCH
                                 || clause_GetTemporalType(PartnerClause) == STEP)
             && tl_IsUniversalAtom(clause_LiteralAtom(NextLit)))) {
          if (clause_LiteralsAreComplementary(ActLit,NextLit)) {
            if (clause_Length(PartnerClause) == 1 ||
                subs_SubsumesBasic(PartnerClause,ActClause,
...
***************************************************************/
{
  ...
  while (i < length) {
    ...
    TERM Atom = clause_LiteralAtom(ActLit);

    if(TEMPORAL_MODE && (clause_GetTemporalType(Copy) == LOOPSEARCH
                         || clause_GetTemporalType(Copy) == STEP) && tl_IsUniversalAtom(Atom)) {
      ++i;
      continue;
    }
...
***************************************************************/
{
  ...
  while (CandTerm) {
    ...
    for ( ; !list_Empty(LitScan); LitScan = list_Cdr(LitScan)) {
      ...
      if(!TEMPORAL_MODE || !((clause_GetTemporalType(PClause) == LOOPSEARCH
                              || clause_GetTemporalType(PClause) == STEP)
                       && tl_IsUniversalAtom(clause_LiteralAtom(PLit)))) {
        if (PClause != Clause &&
...
***************************************************************/
{
  ...
  if (clause_Length(RedClause) == 1) { // the clause cannot be a step clause, so no problems with
    LITERAL ActLit, PLit;              // left-hand side literals
	...
	*Result = list_Cons(Copy, *Result);
    ...
    for (i = clause_FirstLitIndex(); i < length; i++) {
      ...
      if(TEMPORAL_MODE && (clause_GetTemporalType(RedClause) == LOOPSEARCH
                            || clause_GetTemporalType(RedClause) == STEP) && tl_IsUniversalAtom(Atom)) {
        continue;
      }
...
static BOOL red_SimpleStaticReductions(PROOFSEARCH Search, CLAUSE *RedClause, FLAGSTORE Flags,
				       PRECEDENCE Precedence, LIST* Result)
/**********************************************************
...
***********************************************************/
{
  ...
  if (flag_GetFlagValue(Flags, flag_RTAUT) != flag_RTAUTOFF &&
      red_Tautology(Search, *RedClause, Flags, Precedence)) {
    return TRUE;
  }
...
{ 
  ...
  Copy      = (CLAUSE)NULL;
  Redundant = red_SimpleStaticReductions(Search, Clause, Flags, Precedence, Result);
  ...
  if (flag_GetFlagValue(Flags, flag_RFSUB)) {
    *Subsumer = red_ForwardSubsumption(*Clause, Index, Flags, Precedence);
    if ((Redundant = (*Subsumer != (CLAUSE)NULL))) {
      prfs_IncForwardSubsumedClauses(Search, 1);
      return Redundant;
    }
  }
  ...
  while (Rewritten) {
	...
	((StandardRew &&
	 ...
	 (ContextualRew &&
      ...
      Rewritten = TRUE;
      red_ExchangeClauses(Clause, &Copy, Result);
      Redundant = red_SimpleStaticReductions(Search, Clause, Flags,
					     Precedence, Result);
      if (Redundant)
	return Redundant;
      if (clause_IsEmptyClause(*Clause))
    ...
    if (red_UsableMode(Mode))
      Redundant = list_Nconc(Redundant,
			     red_BackSubsumption(Clause,
						 ...
						 Flags, Precedence));
    prfs_IncBackwardSubsumedClauses(Search, list_Length(Redundant));
...
***************************************************************/
{
  ...
  while (!list_Empty(DerivedClauses)) {
    ...
    Clause     = tl_SimplifyLoopSearchMarkers(Search, Clause, Flags, Precedence);
    if(!Clause) {
      continue;
    }
...
static void red_CDBackSubsumption(CLAUSE RedCl, FLAGSTORE Flags,
				  ...
				  st_INDEX Index, PROOFSEARCH Search)
/**********************************************************
...
***********************************************************/
{
  ...
  if (!clause_IsEmptyClause(RedCl)) {
	...
	*UsListPt = list_PointerDeleteOneElement(*UsListPt, SubsumedCl);
    ...
    prfs_IncBackwardSubsumedClauses(Search, list_Length(SubsumedList));
...
***************************************************************/
{
  ...
  if (red_CDForwardSubsumption(RedClause, Index, Flags, Precedence)) {
    ...
    red_CDBackSubsumption(RedClause, Flags, Precedence,
			  ApproxUsListPt, ApproxWOListPt, Index, Search);
...
BOOL red_ClauseDeletion(SORTTHEORY Theory, CLAUSE RedClause, FLAGSTORE Flags,
			PRECEDENCE Precedence, PROOFSEARCH Search)
/**************************************************************
  ...
  if (Theory != (SORTTHEORY)NULL) {
	...
	for (Scan = ApproxDerivables; !list_Empty(Scan) && list_Empty(EmptyClauses);
	  ...
	  if (red_CDReduce(Theory, DerClause, Flags, Precedence,
			   &ApproxUsableList, &ApproxWOList, Index, Search)) 
...
**********************************************************/
{
  ...
  while (!list_Empty(ClauseList) && list_Empty(EmptyClauses) &&
    ...
    Given = red_ReductionOnDerivedClause(Search, Given, red_USABLE);
    if (Given) {
      if (!red_ExplicitTautology(Search, Given,Flags,Precedence)) {
	prfs_IncKeptClauses(Search);
	if (clause_IsEmptyClause(Given))
	  EmptyClauses = list_List(Given);
	else {
