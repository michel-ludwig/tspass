...
typedef enum  {LOOPSEARCH             = 1,
               TERMINATING_LOOPSEARCH = 2,
               // '_TYPE' is necessary to get it to compile with gcc 4.1.2
               INITIAL_TYPE           = 3,
               UNIVERSAL              = 4,
               STEP                   = 5
              } TEMPORAL_TYPE;
...
void    clause_PrintClauseList(LIST);
...
void    clause_ReplaceConstant(CLAUSE, SYMBOL, SYMBOL);
...
LIST    clause_ListOfVariablesInClauseList(LIST);
...
static __inline__ SYMBOL clause_LiteralSymbol(LITERAL Lit)
{
  return term_TopSymbol(clause_LiteralAtom(Lit));
}
...
static __inline__ BOOL clause_IsFromLoopSearch(CLAUSE Clause)
{
  return Clause->origin == LOOP_SEARCH;
}
...
static __inline__ void clause_SetFromLoopSearch(CLAUSE Clause)
{
  Clause->origin = LOOP_SEARCH;
}
...
static __inline__ void clause_SetTemporalType(CLAUSE Clause, TEMPORAL_TYPE Type)
{
  Clause->temporaltype = Type;
}
				     ...
				     PRECEDENCE Precedence)
{
  ...
  if(TEMPORAL_MODE) {
    clause_SetTemporalType(Clause, clause_DetermineTemporalType(Clause));
  }
...
/**
 * Returns 'TRUE' if and only if the clause 'C1' is contained in the clause
 * 'C2'.
 *
 * CAUTION: Variables have to be normalised.
 **/
 ...
 * CAUTION: Variables have to be normalised.
 **/
static __inline__ BOOL clause_Equal(CLAUSE C1, CLAUSE C2)
{
  if(clause_Length(C1) != clause_Length(C2)) {
    return FALSE;
  }

  return (clause_IsContained(C1, C2) && clause_IsContained(C2, C1));
}

static __inline__ TEMPORAL_TYPE clause_GetTemporalType(CLAUSE Clause)
{
  if(!Clause->temporaltype) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In clause_GetTemporalType:");
    misc_ErrorReport(" Tried to access undefined temporal type.");
    misc_FinishErrorReport();    
  }
  return Clause->temporaltype;
}
