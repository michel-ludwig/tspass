...
***********************************************************/
{
  ...
  /* Terminating loop search clauses are only allowed to subsume terminating loop search clauses! */
  if(TEMPORAL_MODE && clause_GetTemporalType(C1) == TERMINATING_LOOPSEARCH
                   && clause_GetTemporalType(C2) != TERMINATING_LOOPSEARCH) {
     return FALSE;
  }

#if 0
  /* We only allow loop search clauses to subsume loop search clauses. */
  if(clause_IsLoopSearchClause(C2) && !clause_IsLoopSearchClause(C1)) {
    return FALSE;
  }
#endif
...
#if 0
  /* There shouldn't be a need to check for loop search clause 
     subsumption as this function only works on a single clause. */
#endif
  vec = vec_ActMax();
