...
void symbol_ReinitOrderingCounter(void)
{
  symbol_ORDERING = 1;
}
...
***************************************************************/
{
  return list_PointerMember(symbol_FREEDSYMBOLS, (POINTER)symbol_Index(Symbol));
}
...
/**************************************************************
  INPUT:   An list of variable symbols.
  RETURNS: A variable symbol.
  SUMMARY: Finds a variable symbol that is not contained in the
           list.
***************************************************************/
SYMBOL symbol_FindUnusedVariable(LIST VariableSymbolList)
{
  SYMBOL toReturn = symbol_FirstVariable();

  if(list_Empty(VariableSymbolList)) {
    return toReturn;
  }

  /* FIXME: this could done better! */
  for(LIST Scan = VariableSymbolList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    SYMBOL Symbol = (SYMBOL)list_Car(Scan);

    if(symbol_GreaterVariable(Symbol, toReturn)) {
      toReturn = Symbol;
    }
  }
  ++toReturn;
...
#ifdef CHECK
  if (toReturn >= symbol__MAXSTANDARDVAR) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In symbol_FindUnusedVariable: Number of standard variables exceeded.\n");
    misc_FinishErrorReport();
  }
#endif
  return toReturn;
