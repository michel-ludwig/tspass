...
TEMPORAL_TYPE clause_DetermineTemporalType(CLAUSE Clause)
{
  return tl_DetermineTemporalType(Clause);
}
...
***********************************************************/
{
  ...
  if(TEMPORAL_MODE
     && (clause_GetTemporalType(Clause) == LOOPSEARCH
         || clause_GetTemporalType(Clause) == STEP)) {
    tl_EnsureNoUniversalLiteralIsMaximal(Clause);
  }
...
void clause_ReplaceConstant(CLAUSE Clause, SYMBOL Const, SYMBOL Repl)
/**********************************************************
  INPUT:   A clause, a constant symbol, and a variable or constant.
  RETURNS: Nothing.
  EFFECTS: All occurences of the variable <Var> in <Clause>
...
***********************************************************/
{
...
#ifdef CHECK
  if (!symbol_IsConstant(Const)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In clause_ReplaceConstant: symbol is not a constant");
    misc_FinishErrorReport();
  }
#endif
  ...
  li = clause_LastLitIndex(Clause);
  for (i = clause_FirstLitIndex(); i <= li; i++)
    term_ReplaceConstant(clause_GetLiteralAtom(Clause,i), Const, Repl);
}
...
void clause_SetLoopSearchFlag(CLAUSE Clause)
{
  int i, li;
  li = clause_LastLitIndex(Clause);

  for (i = clause_FirstLitIndex(); i <= li; i++) {
    if(list_Length(term_ArgumentList(clause_GetLiteralAtom(Clause, i))) == 0) {
      clause_SetFlag(Clause, LOOPSEARCH);
      break;
    }
  }
}
...
LIST clause_ListOfVariablesInClauseList(LIST ClauseList)
{
  ...
  for(LIST Scan = ClauseList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    ...
    toReturn = list_Nconc(clause_ListOfVariables(Clause), toReturn);
  }
...
CLAUSE clause_CreateCrude(LIST Constraint, LIST Antecedent, LIST Succedent,
			  BOOL Con)
/**************************************************************
  INPUT:   Three lists of pointers to terms (!) and a Flag indicating
...
***************************************************************/
{
  ...
  if (!clause_Exists(Clause))
    ...
    if(clause_GetFlag(Clause, LOOPSEARCH)) {
      printf(":LS");
    }
    putchar(']');
...
void clause_PrintClauseList(LIST List)
{
...
***************************************************************/
{
  ...
  case LOOP_SEARCH:                     fputs("LoopSearch", File); break;
  default:
  //FIXME
  #if 0
    misc_StartErrorReport();
    ...
    misc_FinishErrorReport();
  #endif
                                         fputs("No origin given.", File); break;
  }
}
...
static BOOL clause_CheckLiteralRangeEqual(CLAUSE C1, CLAUSE C2, int i1, int j1,
                                                                int i2, int j2)
{
  for(int i = i1; i <= j1; ++i) {
    LITERAL Lit1 = clause_GetLiteral(C1, i);
    TERM Atom1 = clause_LiteralSignedAtom(Lit1);
    BOOL Found = FALSE;

    for(int j = i2; j <= j2; ++j) {
      LITERAL Lit2 = clause_GetLiteral(C2, j);
...
BOOL clause_IsContained(CLAUSE C1, CLAUSE C2)
{
  if(clause_NumOfConsLits(C1) > clause_NumOfConsLits(C2)
     || clause_NumOfAnteLits(C1) > clause_NumOfAnteLits(C2)
     || clause_NumOfSuccLits(C1) > clause_NumOfSuccLits(C2)) {
    return FALSE;
  }
  if(clause_NumOfConsLits(C1) > 0 && !clause_CheckLiteralRangeEqual(C1, C2, clause_FirstLitIndex(), clause_LastConstraintLitIndex(C1),
                                            clause_FirstLitIndex(), clause_LastConstraintLitIndex(C2))) {
    return FALSE;
  }
  if(clause_NumOfAnteLits(C1) > 0 && !clause_CheckLiteralRangeEqual(C1, C2, clause_FirstAntecedentLitIndex(C1), clause_LastAntecedentLitIndex(C1),
                                                                            clause_FirstAntecedentLitIndex(C2), clause_LastAntecedentLitIndex(C2))) {
    return FALSE;
  }
  if(clause_NumOfSuccLits(C1) > 0 && !clause_CheckLiteralRangeEqual(C1, C2, clause_FirstSuccedentLitIndex(C1), clause_LastSuccedentLitIndex(C1),
                                                                            clause_FirstSuccedentLitIndex(C2), clause_LastSuccedentLitIndex(C2))) {
    return FALSE;
  }
  return TRUE;
}
