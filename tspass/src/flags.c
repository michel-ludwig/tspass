...
static void flag_SetInternal(FLAG_ID Flag)
{
  flag_PROPERTIES[Flag].internal = TRUE;
}
...
***************************************************************/
{
 ...
 /* Assignment Equation Deletion output not activated  */
  ...
  flag_InitIntern(flag_PGIVEN, flag_PRINTING, "PGiven", flag_PGIVENOFF,
  ...
  /* Init the loop search test flag */
  flag_InitIntern(flag_LOOPSEARCHTEST, flag_TEMPORALLOGICSPECIAL, "LoopSearchTest", flag_LOOPSEARCHTESTSUBSUMPTION,
      flag_LOOPSEARCHTESTMIN, flag_LOOPSEARCHTESTMAX);

  /* Init the loop search iteration threshold flag */
  flag_InitIntern(flag_LOOPSEARCHITERATIONS, flag_TEMPORALLOGICSPECIAL, "LoopSearchIterations", 10,
      flag_LOOPSEARCHITERATIONSMIN, flag_LOOPSEARCHITERATIONSMAX);

  /* Init the sequential loop search flag */
  flag_InitIntern(flag_SEQUENTIALLOOPSEARCH, flag_TEMPORALLOGICSPECIAL, "SequentialLoopSearch", flag_OFF,
      flag_SEQUENTIALLOOPSEARCHMIN, flag_SEQUENTIALLOOPSEARCHMAX);

  /* Init the printing of deletions for loop search clauses with different markers flag */
  flag_InitIntern(flag_PDIFFMARK, flag_PRINTING, "PDiffMark", flag_OFF,
      flag_PDIFFMARKMIN, flag_PDIFFMARKMAX);

  /* Init the printing of loop search candidate clauses for the loop search test flag */
  flag_InitIntern(flag_PLOOPCAND, flag_PRINTING, "PLoopCand", flag_OFF,
      flag_PLOOPCANDMIN, flag_PLOOPCANDMAX);

  /* Init the model construction flag */
  flag_InitIntern(flag_MODELCONSTRUCTION, flag_TEMPORALLOGICSPECIAL, "ModelConstruction", flag_OFF,
      flag_MODELCONSTRUCTIONMIN, flag_MODELCONSTRUCTIONMAX);

  /* Init the sequential loop search iteration flag */
  flag_InitIntern(flag_SEQUENTIALLOOPSEARCHITERATIONS, flag_TEMPORALLOGICSPECIAL, "SeqLoopSearchIterations", 100,
      flag_SEQUENTIALLOOPSEARCHITERATIONSMIN, flag_SEQUENTIALLOOPSEARCHITERATIONSMAX);

  /* Init the model construction step printing flag */
  flag_InitIntern(flag_PMODELCONSTRUCTIONSTEPS, flag_PRINTING, "PModelConstructionSteps", flag_OFF,
      flag_PMODELCONSTRUCTIONSTEPSMIN, flag_PMODELCONSTRUCTIONSTEPSMAX);

  /* Init the model construction type flag */
  flag_InitIntern(flag_MODELCONSTRUCTIONTYPE, flag_TEMPORALLOGICSPECIAL, "ModelConstructionType", flag_ORDEREDMODELCONSTRUCTION,
      flag_MODELCONSTRUCTIONTYPEMIN, flag_MODELCONSTRUCTIONTYPEMAX);

  /* Init the unordered fine-grained resolution model construction flag */
  flag_InitIntern(flag_UNORDEREDFINEGRAINEDSTEPRESOLUTION, flag_TEMPORALLOGICSPECIAL, "UnorderedFineGrainedStepResolution", flag_UNORDEREDFINEGRAINEDSTEPRESOLUTIONOFF,
      flag_UNORDEREDFINEGRAINEDSTEPRESOLUTIONMIN, flag_UNORDEREDFINEGRAINEDSTEPRESOLUTIONMAX);
  flag_SetInternal(flag_UNORDEREDFINEGRAINEDSTEPRESOLUTION);

  /* Init the elimination of redundant cycles flag */
  flag_InitIntern(flag_ELIMINATEREDUNDANTCYCLES, flag_TEMPORALLOGICSPECIAL, "EliminateRedundantCycles", flag_ON,
      flag_ELIMINATEREDUNDANTCYCLESMIN, flag_ELIMINATEREDUNDANTCYCLESMAX);
}
...
{
  return flag_PROPERTIES[Flag].internal;
}
