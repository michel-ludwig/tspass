...
static TERM cnf_MakeSkolemFunction(LIST varlist, PRECEDENCE Precedence, LIST *Symblist)
/**************************************************************
...
static TERM cnf_MoveProvedTermToTopLevel(TERM Term, TERM Term1, TERM Proved,
					 LIST VarList, LIST FreeList, LIST* Symblist, 
					 PRECEDENCE Precedence)
/********************************************************************
...
*******************************************************************/
{
  ...
  for (scan = VarList; scan != list_Nil(); scan = list_Cdr(scan)) {
    ...
    skolemfunction = cnf_MakeSkolemFunction(FreeList, Precedence, Symblist);
...
static void cnf_SkolemFunctionFormulaMapped(TERM term, LIST allist, LIST map, FLAGSTORE Flags)
/**************************************************************
...
static BOOL cnf_HasDeeperVariable(LIST List1, LIST List2, FLAGSTORE Flags)
/******************************************************************
      ...
      while (!list_Empty(pairscan)) {
        ...
        if (currentlength < pairlength) {
          ...
          if (cnf_HasDeeperVariable((LIST) list_PairSecond((LIST) list_Car(pairscan)), /* in list */
                                    (LIST) list_PairSecond(pair), Flags)) {                   /* new pair */
#ifdef CHECK_STRSKOLEM
            if (flag_GetFlagValue(Flags, flag_PSTRSKOLEM)) {
              fputs("\nTerm ", stdout);
	  ...
	  if (Result1 || result2) {
	    ...
	    provedterm =
	      cnf_MoveProvedTermToTopLevel(topterm, term, list_Car(scan), 
					   varlist, freevariables, Symblist, Precedence);
	    if (flag_GetFlagValue(Flags, flag_POPTSKOLEM)) {
	      fputs("Result : ", stdout);
	      term_Print(topterm);
	      putchar('\n');
...
/**
 * WARNING: no temporal formulae are allowed in the conjecture!
 **/
PROOFSEARCH cnf_Flotter(LIST AxiomList, LIST ConjectureList,
      LIST* AllLabels, LIST* InitialClauses, LIST* UniversalClauses,
      LIST *StepClauses, LIST *EventualityClauses, HASH TermLabelToClauselist,
      HASH ClauseToTermLabellist, FLAGSTORE InputFlags, PRECEDENCE InputPrecedence,
      LIST* Symblist, BOOL* predicateMap)
/**************************************************************
...
***************************************************************/
{
  LIST        Scan, Scan2, SkolemFunctions;
//   LIST        FormulaClauses;
  LIST        SkolemPredicates, AllFormulae;
//   LIST        EmptyClauses;
  LIST        UsedTerms;
  TERM        AxTerm;
//   TERM        Formula;
  ...
  for (Scan = AxiomList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    LIST Pair;
    Pair = list_Car(Scan);
    TERM RootTerm = (TERM) list_PairSecond(Pair);
    AxTerm = RootTerm;
    DSNF_TYPE Type = tl_FormulaType(AxTerm);

    if(Type != INITIAL_FORMULA && Type != UNIVERSAL_FORMULA) {
      continue;
    }
    if(Type == UNIVERSAL_FORMULA) {
      AxTerm = term_FirstArgument(AxTerm);
    }
    ...
    if(Type == UNIVERSAL_FORMULA) {
      term_RplacFirstArgument(RootTerm, AxTerm);
    }
    else {
      list_PairRplacSecond(Pair, AxTerm);
    }
  }
...
/* disabling this for temporal logic as input saturation is not possible on untranslated
   clauses */
  ...
  *Symblist = list_Nil();
  for (Scan = AllFormulae; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    LIST Ax = list_Nil(), Pair;
    UsedTerms = list_Nil();
    Pair = list_Car(Scan);
    TERM TemporalFormula = (TERM) list_PairSecond(Pair);
    DSNF_TYPE Type = tl_FormulaType(TemporalFormula);
    TERM NewFormula = NULL;
    if(Type == INITIAL_FORMULA || Type == UNIVERSAL_FORMULA) {
      TERM Formula = (Type == UNIVERSAL_FORMULA ? term_FirstArgument(TemporalFormula)
                                                : TemporalFormula);
  #ifdef CHECK_CNF
      fputs("\nFormula : ", stdout);
  ...
  #endif
// disabling this as there seems to be a bug in 'cnf_StrongSkolemization'!
// for the formula 'always ![X] (QNK2M2(X) -> (?[Y] (R2(X,Y) & ~m2(Y))))'
//      Ax = cnf_OptimizedSkolemization(Search, term_Copy(Formula),
//                                      (char*) list_PairFirst(Pair), &UsedTerms,
//                                      Symblist,Result,FALSE,InputClauseToTermLabellist);
      NewFormula = cnf_SkolemFormula(term_Copy(Formula), Flags, Precedence, Symblist);
      NewFormula = cnf_DistributiveFormula(NewFormula);
      Ax = cnf_MakeClauseList(NewFormula, FALSE, FALSE, Flags, Precedence);
      term_Delete(NewFormula);
      if(Type == UNIVERSAL_FORMULA) {
        *UniversalClauses = list_Nconc(*UniversalClauses, Ax);
      }
      else {
        *InitialClauses = list_Nconc(*InitialClauses, Ax);
      }
    }
    else if(Type == STEP_FORMULA) {
       Ax = list_List(tl_ClausifyAndTemporaliseStepFormula(TemporalFormula, predicateMap,
                                                           Flags, Precedence));
       *StepClauses = list_Nconc(*StepClauses, Ax);
    }
    else if(Type == EVENTUALITY_FORMULA) {
      BOOL Unsatisfiable;
      CLAUSE EventualityClause = tl_ClausifyAndTemporaliseEventualityFormula(TemporalFormula,
                                                                             predicateMap, &Unsatisfiable, Flags, Precedence);
      if(EventualityClause) {
        *EventualityClauses = list_Cons(EventualityClause, *EventualityClauses);
      }
      else if(Unsatisfiable) {
        // add an empty clause
        *UniversalClauses = list_Cons(clause_Create(list_Nil(), list_Nil(), list_Nil(), Flags, Precedence), *UniversalClauses);
      }
    }
    else {
      misc_StartErrorReport();
      misc_ErrorReport("\n In cnf_Flotter: UNDEFINED found.\n");
      misc_FinishErrorReport();
    }
...
/* FIXME: there seems to be a bug if Term = true */
LIST cnf_QueryFlotter(PROOFSEARCH Search, TERM Term, LIST* Symblist, BOOL NegateInputTerm)
/**************************************************************
...
***************************************************************/
{
  ...
  /* Transfer the precedence into the local search object again as there might be
  ...
  /* also return the created Skolem functions and predicates */
  *Symblist = list_Nconc(SkolemPredicates, *Symblist);
  *Symblist = list_Nconc(SkolemFunctions, *Symblist);
  term_Delete(Term);
  /* Transfer the precedence into the local search object again as there might be
     new Skolem functions */
  symbol_TransferPrecedence(prfs_Precedence(cnf_SEARCHCOPY), Precedence);
...
**********************************************************/
{
clause_DeleteClauseList(ClauseList); // don't forget this!
return list_Nil();
// don't allow for any saturation during the cnf transformation
#if 0
  CLAUSE     Given;
