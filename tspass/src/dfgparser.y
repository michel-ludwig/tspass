...
/* Extension for temporal logics */
%token DFG_ALWAYS DFG_SOMETIME DFG_NEXT DFG_EVENTUALITY
...
%type <symbol>   binsymbol nsymbol quantsymbol tempsymbol
...
formula:	atom		{ $$ = $1; }
		...
		| tempsymbol '(' formula ')'
		  { $$ = dfg_IGNORE ? NULL : term_Create($1, list_List($3)); }
...
tempsymbol:	DFG_ALWAYS		{ $$ = tl_Always(); }
		| DFG_SOMETIME		{ $$ = tl_Sometime(); }
		| DFG_NEXT		{ $$ = tl_Next();  }
;
...
cnfclausesopt:	/* empty */		{ $$ = list_Nil(); }
		...
		| cnfclausesopt DFG_EVENTUALITY '(' cnfclauseopt labelopt ')' '.'
		  { LIST pair;
		    if ($4 == NULL) { /* No clause */
		      if ($5 != NULL)
			string_StringFree($5);
		      ...
		      pair = list_PairCreate($5, $4);
		      dfg_EVCLAUSES = list_Nconc(dfg_EVCLAUSES, list_List(pair));
		      $$ = $1;
		    }
		    dfg_VarCheck();
		  }
...
LIST dfg_DFGParser(FILE* File, FLAGSTORE Flags, PRECEDENCE Precedence,
		   LIST* Axioms, LIST* Conjectures, LIST *EventualityClauses, LIST* SortDecl,
...
***************************************************************/
{
  ...
  for (scan = dfg_EVCLAUSES; !list_Empty(scan); scan = list_Cdr(scan)) {
    tupel = list_Car(scan);
    clauseTerm = list_PairSecond(tupel);
    list_Rplaca(scan, dfg_CreateClauseFromTerm(clauseTerm, FALSE, Flags, Precedence));
    if (list_PairFirst(tupel) != NULL)        /* Label is defined */
  ...
  /* Since dfg_CreateClauseFromTerm() returns NULL for trivial tautologies */
  /* we now delete those NULL pointers from the clause list.               */
  dfg_EVCLAUSES = list_PointerDeleteElement(dfg_EVCLAUSES, NULL);
  ...
  *EventualityClauses    = dfg_EVCLAUSES;
