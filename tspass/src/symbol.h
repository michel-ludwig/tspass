...
typedef enum {SKOLEM=1,    CUMMUTATIVE=2, ASSOCIATIVE=4, ORDRIGHT=8, ORDMUL=16,
	      ...
	      PREDEFINED=2048, LOOPSEARCHMARKER = 4096
} SPROPERTY;
...
SYMBOL          symbol_FindUnusedVariable(LIST);
/**************************************************************/
/* Symbol Comparisons                                         */
/**************************************************************/
...
static __inline__ SYMBOL symbol_SecondVariable(void)
{
  return 2;
}
...
static __inline__ BOOL symbol_LessVariable(SYMBOL Var1, SYMBOL Var2)
{
  return Var1 < Var2;
}
...
static __inline__ BOOL symbol_IsLoopSearchMarker(SYMBOL Symbol)
{
  return (symbol_HasProperty(Symbol, LOOPSEARCHMARKER));
}
...
static __inline__ PRECEDENCE symbol_CopyPrecedence(PRECEDENCE P)
{
  PRECEDENCE toReturn = symbol_CreatePrecedence();
  symbol_TransferPrecedence(P, toReturn);
